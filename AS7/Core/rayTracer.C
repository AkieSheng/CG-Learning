...
Vec3f RayTracer::mirrorDirection(Vec3f const&normal,
                                 Vec3f const&incoming) const {
  float nDotI = normal.Dot3(incoming);
  return incoming - normal * (2.0f * nDotI);
}

bool RayTracer::transmittedDirection(Vec3f const&normal,
...
Vec3f RayTracer::prepareNormal(Ray const&ray, Hit const&hit,
...
bool RayTracer::castSceneShadow(Ray const&ray, float tmin, float tmax, float &t,
...
bool RayTracer::rayCastShadow(Ray const&ray, float tmin, float tmax, float &t,
                              Material **outMaterial) const {
  const Object3DVector &infinite = grid->getInfiniteObjects();
...
Vec3f RayTracer::getShadowAttenuation(Vec3f const&point,
...
Vec3f RayTracer::computeLocalShading(Ray const&ray, Hit const&hit,
