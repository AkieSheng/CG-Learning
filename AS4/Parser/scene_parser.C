...
auto SceneParser::parsePointLight() -> Light* {
  char token[MAX_PARSER_TOKEN_LENGTH];
  getToken(token);
  assert(!::strcmp(token, "{"));
  getToken(token);
  assert(!::strcmp(token, "position"));
  auto position = readVec3f();
  getToken(token);
  assert(!::strcmp(token, "color"));
  auto color = readVec3f();
  float att[3] = {1, 0, 0};
...
auto SceneParser::parsePhongMaterial() -> Material* {
  char token[MAX_PARSER_TOKEN_LENGTH];
  auto diffuseColor = Vec3f(1, 1, 1);
  auto specularColor = Vec3f(0, 0, 0);
  auto exponent = 1.0f;
  auto reflectiveColor = Vec3f(0, 0, 0);
  auto transparentColor = Vec3f(0, 0, 0);
  auto indexOfRefraction = 1.0f;
  getToken(token);
  assert(!::strcmp(token, "{"));
  while (1) {
    getToken(token);
...
auto SceneParser::parseTriangleMesh() -> Group* {
  ...
  while (1) {
    ...
    if (c == EOF) {
      ...
      assert(current_material != nullptr);
      auto* t = new Triangle(verts[f0 - 1], verts[f1 - 1], verts[f2 - 1],
                             current_material);
