...
auto PhongMaterial::Shade(Ray const& ray, Hit const& hit,
  ...
  auto viewDir = ray.getDirection() * (-1.0f);
  viewDir.Normalize();
...
auto PhongMaterial::glSetMaterial() const -> void {
  ...
  GLfloat specular[4] = {getSpecularColor().x(), getSpecularColor().y(),
                       getSpecularColor().z(), 1.0};
  GLfloat diffuse[4] = {getDiffuseColor().x(), getDiffuseColor().y(),
                        getDiffuseColor().z(), 1.0};

  auto glexponent = exponent;
  if (glexponent < 0.0f) {
    glexponent = 0.0f;
  }
  if (glexponent > 128.0f) {
    glexponent = 128.0f;
  }
